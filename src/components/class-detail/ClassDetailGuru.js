...
import { Users, BookOpen, Award, AlertTriangle, ArrowLeft, Printer, Plus, Edit, Trash2 } from 'lucide-react';
...
export default function ClassDetailGuru({ kelasId }) {
  ...
  const handleAddTask = () => { setEditTask(null); setShowTaskModal(true); };
  const handleEditTask = (taskId) => { setEditTask(tasks.find(t => t._id === taskId)); setShowTaskModal(true); };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6">
      {showAddStudent && (
        <AddStudentModal kelasId={kelasId} onSuccess={() => setRefreshKey(k => k + 1)} onClose={() => setShowAddStudent(false)} />
      )}
      {showBulkAddStudent && (
        <BulkAddStudentModal kelasId={kelasId} onSuccess={() => setRefreshKey(k => k + 1)} onClose={() => setShowBulkAddStudent(false)} />
      )}
      {showTaskModal && (
        <TaskModal kelasId={kelasId} initialData={editTask} onSuccess={() => setRefreshKey(k => k + 1)} onClose={() => setShowTaskModal(false)} />
      )}
      {toast.message && (
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={handleAddStudent} className="inline-flex items-center px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm">
                <Plus className="w-4 h-4 mr-1" /> Tambah
              </button>
              <button onClick={handleBulkAddStudent} className="inline-flex items-center px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm">
                <Plus className="w-4 h-4 mr-1" /> Bulk
              </button>
            </div>
          </div>
          {loadingStudents ? (
                      ...
                      <tr key={s._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-4 py-3 text-center">
                          {isAtRisk ? (
                            ...
                            <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Baik</span>
                          )}
                        </td>
                      </tr>
                    );
      ...
      {activeTab === 'tugas' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={handleAddTask} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
          ...
          {tasks.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {tasks.map(t => {
                ...
                return (
                  <div key={t._id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 flex flex-col gap-2 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-lg text-blue-700 dark:text-blue-300 truncate">{t.judul}</span>
                      <span className={`px-2 py-0.5 text-xs rounded font-semibold ${isOverdue ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                        {isOverdue ? 'Lewat' : 'Aktif'}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500">
                      Deadline: {t.tanggal_deadline ? new Date(t.tanggal_deadline).toLocaleDateString('id-ID') : '-'}
                    </p>
                    <p className="text-sm text-gray-500">Terkumpul: {submitted}/{totalSiswa}</p>
                    <div className="flex gap-2 mt-auto pt-2 border-t border-gray-100 dark:border-gray-700">
                      <button onClick={() => handleEditTask(t._id)} className="text-blue-600 hover:text-blue-800 flex items-center gap-1 text-sm">
                        <Edit className="w-4 h-4" /> Edit
                      </button>
                      <button onClick={() => handleDeleteTask(t._id)} className="text-red-600 hover:text-red-800 flex items-center gap-1 text-sm">
                  ...
                  <tr>
                    ...
                    {tasks.map(t => (
                      <th key={t._id} className="px-4 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">
                        {t.judul.length > 12 ? t.judul.substring(0, 12) + '...' : t.judul}
                      </th>
                    ))}
